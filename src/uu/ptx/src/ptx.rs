...
impl Line<'_> {
    ...
    /// `range` with whitespace dropped from both ends.
...
impl Chunks {
    ...
    fn new(config: &Config, all_before: &[char], keyword: String, all_after: &[char]) -> Self {
        ...
        // deliberate, but every other width here counts characters. It only
        // shows on input outside ASCII, where ptx already diverges from GNU.
...
mod tests {
    ...
    fn trim_drops_only_the_outer_whitespace() {
        ...
        // A window holding nothing but whitespace collapses.
        assert!(line.trim(12..14).is_empty());
    }
