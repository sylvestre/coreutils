        ...
        if fperm == mode {
            if self.verbose && !self.changes {
                println!(
                    "{}",
                    get_message_with_args(
                        "chmod-verbose-mode-retained",
                        HashMap::from([
                            ("file".to_string(), file.quote().to_string()),
                            ("mode_octal".to_string(), format!("{fperm:03o}")),
                            (
            ...
            if self.verbose {
                println!(
                    "{}",
                    get_message_with_args(
                        "chmod-verbose-failed-change",
                        HashMap::from([
                            ("file".to_string(), file.quote().to_string()),
                            ("old_mode".to_string(), format!("{fperm:04o}")),
                            (
                                "old_mode_display".to_string(),
                                display_permissions_unix(fperm as mode_t, false)
                            ),
                            ("new_mode".to_string(), format!("{mode:04o}")),
                            (
                                "new_mode_display".to_string(),
