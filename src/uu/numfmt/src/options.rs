...
impl FromStr for FormatOptions {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let padding_start = iter.peek().map_or(s.len(), |&(i, _)| i);
        ...
        if !padding.is_empty() {
            if let Ok(p) = padding.parse() {
                ...
                return Err(error(
                    ...
                    padding_start..iter.peek().map_or(s.len(), |&(i, _)| i),
        ...
        if let Some((_, '.')) = iter.peek() {
            ...
            let precision_start = iter.peek().map_or(s.len(), |&(i, _)| i);
            while let Some((_, c)) = iter.peek() {
                if c.is_ascii_digit() {
                    precision.push(*c);
                    iter.next();
            ...
            if precision.is_empty() {
                ...
                return Err(error(
                    translate!("numfmt-error-invalid-precision", "format" => s),
                    precision_start..iter.peek().map_or(s.len(), |&(i, _)| i),
                    FormatErrorKind::NumberOverflow,
                ));
            }
        }
