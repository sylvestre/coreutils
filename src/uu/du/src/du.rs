...
// spell-checker:ignore fstatat openat
...
fn safe_du(
    ...
    // Get initial stat for this path
    let my_stat = match Stat::new(path, None, options) {
        Ok(s) => s,
        Err(e) => {
            let error =
                e.map_err_context(|| translate!("du-error-cannot-access", "path" => path.quote()));
            // Send error to channel - this mimics the non-Linux behavior
            if let Err(send_error) = print_tx.send(Err(error)) {
                return Err(Box::new(send_error));
            }
            // Return an error indicating the error was already handled
            return Err(Box::new(mpsc::SendError(Err(USimpleError::new(
                0, // Use code 0 to indicate "error already handled"
...
fn safe_du_impl(
    ...
    let dir_fd = if let Some(parent) = parent_fd {
        // We're traversing and have a parent fd - use openat
        match parent.open_subdir(path.file_name().unwrap_or(path.as_os_str())) {
            Ok(fd) => fd,
            Err(e) => {
        ...
        // Initial directory - open directly
    ...
    'file_loop: for entry_name in entries {
        ...
        let this_stat = if is_dir {
            // For directories, recurse using safe_du_impl
            Stat {
...
// Internal version with ancestor tracking and symlink depth
fn du_regular_with_ancestors(
    mut my_stat: Stat,
    options: &TraversalOptions,
    depth: usize,
    seen_inodes: &mut HashSet<FileInfo>,
    print_tx: &mpsc::Sender<UResult<StatPrintInfo>>,
    ancestors: &mut HashSet<FileInfo>,
    symlink_depth: usize,
) -> Result<Stat, Box<mpsc::SendError<UResult<StatPrintInfo>>>> {
                    ...
                    match Stat::new(&entry_path, Some(&entry), options) {
                        Ok(this_stat) => {
                            ...
                            if this_stat.metadata.is_dir() {
                                ...
                                let this_stat = du_regular_with_ancestors(
                                    this_stat,
                                    options,
                                    depth + 1,
                                    seen_inodes,
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    'loop_file: for path in files {
        ...
        #[cfg(target_os = "linux")]
        {
            // Use safe traversal on Linux with openat, but not when -L is used
            // because -L requires following symlinks which may point outside the tree
            if traversal_options.dereference == Deref::All {
                // Use regular traversal when -L is specified
                if let Ok(stat) = Stat::new(&path, None, &traversal_options) {
                    let mut seen_inodes: HashSet<FileInfo> = HashSet::new();
                    if let Some(inode) = stat.inode {
                        seen_inodes.insert(inode);
                    }
                    let stat = du_regular(stat, &traversal_options, 0, &mut seen_inodes, &print_tx)
