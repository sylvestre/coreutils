...
fn parse_path_args(
    ...
    if paths.is_empty() {
        ...
        return Err(
            translate!("cp-error-missing-destination-operand", "source" => paths[0].quote()).into(),
        );
    }
    ...
    if options.no_target_dir && options.target_dir.is_none() && paths.len() > 2 {
        return Err(translate!("cp-error-extra-operand", "operand" => paths[2].quote()).into());
    }
...
fn construct_dest_path(
    ...
    if options.no_target_dir && target.is_dir() {
        return Err(translate!("cp-error-cannot-overwrite-directory-with-non-directory", "dir" => target.quote())
        .into());
    }
    ...
    #[cfg(not(windows))]
    {
        std::os::unix::fs::symlink(source, dest).map_err(|e| {
            CpError::IoErrContext(
                e,
                translate!("cp-error-cannot-create-symlink", "dest" => get_filename(dest).unwrap_or("?"), "source" => get_filename(source).unwrap_or("?"))
            )
        })?;
    }
    #[cfg(windows)]
...
fn handle_existing_dest(
    ...
    if is_forbidden_to_copy_to_same_file(source, dest, options, source_in_command_line) {
        return Err(
            translate!("cp-error-same-file", "source" => source.quote(), "dest" => dest.quote())
                .into(),
        );
    }
    ...
    match options.copy_mode {
        CopyMode::Link => {
            ...
            .map_err(|e| {
                CpError::IoErrContext(
                    e,
                    translate!("cp-error-cannot-create-hard-link", "dest" => get_filename(dest).unwrap_or("?"), "source" => get_filename(source).unwrap_or("?"))
                )
            })?;
        }
        CopyMode::Copy => {
