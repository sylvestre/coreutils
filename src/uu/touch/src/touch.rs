...
pub fn uumain(args: impl uucore::Args) -> i32 {
    ...
    .arg(
        Arg::with_name(OPT_TIME)
            .long(OPT_TIME)
            .help("change only the specified time: \"access\", \"atime\", or \
    ...
    for filename in &files {
        ...
        if !Path::new(path).exists() {
            ...
            if !(matches.is_present(OPT_DATE) || matches.is_present(OPT_REFERENCE) || matches.is_present(OPT_CURRENT)) {
                continue;
            }
        }
        ...
        if matches.is_present(OPT_ACCESS) || matches.is_present(OPT_MODIFICATION) || matches.is_present(OPT_TIME)
         {
