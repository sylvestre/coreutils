...
//! output — a script, a test suite — still sees the plain one-line message.
//! `UU_DIAG` overrides that decision in either direction.
//!
//! ```text
...
/// Name of the variable that turns rich errors on.
const ENV_VAR: &str = "UU_DIAG";

/// Whether errors should be rendered against their argument list.
///
/// A person at a terminal gets the rich form; anything reading stderr gets the
/// plain message it can grep for. `UU_DIAG` forces one or the other.
///
...
impl Snapshot {
    ...
    /// Build a snapshot of arguments held as raw bytes, as produced by
    /// [`crate::os_string_to_vec`].
    pub fn from_bytes<S: AsRef<[u8]>>(args: &[S]) -> Self {
        let mut snapshot = Self::with_capacity(args.len());
        for arg in args {
            snapshot.push(os_string_from_bytes(arg.as_ref()));
        }
        snapshot
    }
    ...
    /// no longer know where it came from. A repeated value resolves to its first
    /// occurrence.
    ...
    pub fn index_of_bytes(&self, arg: &[u8]) -> Option<usize> {
        self.index_of(&os_string_from_bytes(arg))
    }
    ...
    ///
    /// `message`, `label` and `help` must already be localized. Returns `false`
    /// if nothing could be rendered, in which case the caller should fall back
    /// to a plain one-line message.
...
fn os_string_from_bytes(bytes: &[u8]) -> OsString {
    // Where `os_string_to_vec` produced these bytes it also guaranteed UTF-8.
