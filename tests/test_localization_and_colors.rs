...
fn get_utilities_to_skip() -> HashSet<&'static str> {
    ...
    // Utilities that still need to be updated (can be removed as they get fixed)
    skip_set.insert("pinky"); // Platform-specific utilities that may not be updated yet
    skip_set.insert("who"); // Platform-specific utilities that may not be updated yet
    skip_set.insert("tty"); // Simple utilities that may not need color updates
    skip_set.insert("printenv"); // Simple utilities that may not need color updates
    skip_set.insert("basenc"); // Still using old error handling - needs update
    skip_set.insert("chgrp"); // Still using old error handling - needs update
    skip_set.insert("echo"); // May not show standard error messages for invalid options
    skip_set.insert("chown"); // Still using old error handling - needs update
    skip_set.insert("env"); // Still using old error handling - needs update
    skip_set.insert("chroot"); // Still using old error handling - needs update
    skip_set.insert("expr"); // May not show standard error messages for invalid options
    skip_set.insert("dir"); // Still using old error handling - needs update (alias for ls)
    skip_set.insert("printf"); // May not show standard error messages for invalid options

    // Many utilities still need to be updated to use handle_clap_result()
    // These can be removed from the skip list as they get updated
    // A few utilities still need special handling or have complex error logic
    let utilities_needing_special_handling = [
        "seq", "mktemp", "tail", "truncate", "uniq", "stty",
        "vdir", // These have complex error handling patterns
    ];

    for utility in &utilities_needing_special_handling {
